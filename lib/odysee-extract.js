...
for (const i of items) {
    ...
    /*if (!i.value.title.includes("Umění přes čáru")) {
        continue
    }*/
    //console.log(i)

    const odyseeId = i.short_url.replace(/^lbry:\/\//, '');
    const id = odyseeId.replace(/#[\w\d]+$/, '');

    if (out.find(x => x.id === id)) {
        continue;
    }

    const url =  `https://odysee.com/${odyseeId}`;
    const people = [];
    ...
    out.push({
        ...
        odyseeId: "@paralelnipolis#6/"+odyseeId,
        ...
        name: realName,
        desc: i.value.description && i.value.description !== "\"No description available.\"" ? removeQuotes(i.value.description) : "",
        project: target,
        people,
        lang: "cs",
    })
