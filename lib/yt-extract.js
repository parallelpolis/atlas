...
const matrix = [
    ...
    // example: "Creating the Cypherpunk Future we'd want to live in" by Kurt Opsahl // ECC#2 - Buenos Aires 2025
    ['w3pn-events', null, /^"([^"]+)" by (.+) \/\/ ECC#2 - Buenos Aires 2025$/, [1, 2]], 
    ['w3pn-events', null, /^([^\|]+) - ([^\|]+) \| Web3Privacy Now$/, [1, 2]],
    //['w3pn-events', 'w3pn-eth-cypherpunk-congress-2024', /^([^-]+) by ([^-]+) x Ethereum Cypherpunk Congress$/, [2, 1]],
    ["konference-sp", null, /^([^\:]+)\: (.+) \(KSP([^\)]+)\)$/, [2, 1]],
    ['ethberlin', null, /^ETHBerlin04 Keynote: ([^-]+) - (.+)$/, [2, 1]],
    ['ethberlin', null, /^ETHBerlin04 Fireside: ([^-]+) - (.+)$/, [2, 1]],
    ['protocol-berg', null, /^(Protocol Berg|Protocol Berg Keynote): ([^-]+) - (.+)$/, [3, 2]],
    ['monerokon', null, /^MoneroKon 2019 - ([^\(]+) \(([^\.]+)\)$/, [1, 2]],
    [null, null, /^([^\/]+) \/ ([^\/]+)$/, [1, 2]],
    [null, null, /^([^\|]+) - ([^\|]+)$/, [2, 1]],
    [null, null, /^([^\|]+) \| ([^\|]+)$/, [1, 2]],
]
