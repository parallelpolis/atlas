...
const output = await atlas.dump({ generatedAt: now })
...
const globalMetaFn = join(destDir, '_meta.json');
fs.writeFileSync(globalMetaFn, JSON.stringify({
    generatedAt: now
}, null, 2))
console.log(`Writed: ${globalMetaFn}`)